...
[![Version](https://img.shields.io/badge/Version-v2.3.0-orange.svg)](CHANGELOG.md)
...
- **分析 CSV 匯出**：每格含狀態（已分配／空位／阻擋）、是否前排標記、左鄰、右鄰；空位與阻擋一併列出，方便對應課室。海報請用 A4 列印，不要用 CSV 代替
...
- **課室模板**：名稱、行列、阻擋存於瀏覽器；模板不含學生名單
- **上次座位記憶**：依班別存於本機（session + localStorage），僅座位座標
- **A4 列印**：黑板在上、班名、日期、格內姓名、圖例
