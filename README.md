...
[![Version](https://img.shields.io/badge/Version-v2.7.0-orange.svg)](CHANGELOG.md)
...
- **A4 橫向 PDF**：`班/科座位表(室)`、每一格都畫出來、格內三行（姓名、學號、職位）、右下教師桌、左方班主任、最下方黑板；畫面、列印、PDF 內容相同
...
4. 匯入新名單會自動排位；匯入「座位分析」表會回到原來的座位。或按「建立班內範例」（36 人）
5. 若沒上座，按「把名單放到座位」
6. 右側可加「不可同坐」；下週再按一次「把名單放到座位（智能／週更）」會盡量換位
7. 「匯出橫向 PDF」；需要 Excel 分析則「匯出分析 CSV」
