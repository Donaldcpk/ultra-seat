...
[![Version](https://img.shields.io/badge/Version-v2.4.0-orange.svg)](CHANGELOG.md)
...
- 一鍵列印學校正式「班/科座位表 (室)」A4 橫向表，代課老師也看得懂
- 課室布局模板存本機，不用每次重擋桌
...
- **CSV 匯入**：建議 `班別,班號/學號,姓名[,職務][,前排/SEN]`；相容舊格式 `學號,姓名,職務`
...
- **A4 列印**：學校正式「班/科座位表 (室)」橫向表；中間走道、前方在下、教師桌；螢幕預覽與列印同一份表格
...
2. 填班別、室、行列（預設 6×6）→「建立／套用課室」；班名／室改了標題會立刻更新
3. 切「阻擋」點壞桌 →「儲存目前布局」成模板（下次直接載入）
4. 匯入 CSV，或按「建立班內範例」（36 人）
5. 右側加「不可同坐」（範例會自動帶 3 組）
6. 「智能分配（含週更）」→ 看「分配檢查報告」→「列印 A4 座位表」
7. 下週再按一次智能分配，會盡量換位並告知換了多少人；需要 Excel 分析則「匯出分析 CSV」
...
2. 按「列印 A4 座位表」
3. 選 **A4 橫向**；若格線偏淡，在列印設定打開「背景圖形」
4. 應見：`班/科座位表 (室)` 標題、中間走道、格內姓名、下方教師桌與 Class Teacher

## 私隱

- 純前端；名單與座位只在瀏覽器
- 模板只含格網、室、班主任欄與阻擋，**不含**學生名單
- 公用電腦用完請關閉分頁
