...
from ..utils.simple_file_handler import SimpleFileHandler
...
class SimpleMainWindow:
    ...
    def setup_window(self):
        """設定主視窗屬性"""
        self.root.title("終極智能教室座位表生成器 v1.0 (簡化版)")
    ...
    def create_toolbar(self):
        ...
        ttk.Button(file_frame, text="匯出座位表", 
                  command=self.export_seating_chart).pack(side=tk.LEFT, padx=5, pady=5)
    ...
    def create_right_panel(self, parent):
        ...
        class_entry.bind('<Return>', self.update_class_name)
        ...
        # 建立可捲動的座位表
        canvas_frame = ttk.Frame(seat_frame)
        canvas_frame.pack(fill=tk.BOTH, expand=True)
        
        self.seat_canvas = tk.Canvas(canvas_frame, bg='white')
        v_scrollbar = ttk.Scrollbar(canvas_frame, orient=tk.VERTICAL, command=self.seat_canvas.yview)
        h_scrollbar = ttk.Scrollbar(canvas_frame, orient=tk.HORIZONTAL, command=self.seat_canvas.xview)
        
        self.seat_canvas.configure(yscrollcommand=v_scrollbar.set, xscrollcommand=h_scrollbar.set)
        ...
        # 綁定事件
        self.seat_content_frame.bind('<Configure>', self.on_seat_frame_configure)
    ...
    def create_sample_file(self):
        """建立範例 CSV 檔案"""
        sample_path = 'src/data/sample_students.csv'
    ...
    def export_seating_chart(self):
        """匯出座位表"""
        if not self.classroom:
            messagebox.showwarning("警告", "請先建立教室")
            return
        ...
        try:
            errors = SimpleFileHandler.export_seating_chart_csv(self.classroom, file_path)
            
            if errors:
                messagebox.showerror("匯出錯誤", "\n".join(errors))
            else:
                messagebox.showinfo("匯出完成", f"座位表已匯出到:\n{file_path}")
                self.update_status("座位表匯出完成")
                
        except Exception as e:
            messagebox.showerror("匯出錯誤", f"匯出座位表時發生錯誤:\n{str(e)}")
    ...
    def update_class_name(self, event=None):
        """更新班級名稱"""
        if self.classroom:
            self.classroom.class_name = self.class_name_var.get()
    ...
    def update_classroom_info(self):
        ...
        if self.classroom.blocked_seats:
            info += f"阻擋座位: {len(self.classroom.blocked_seats)}\n"
